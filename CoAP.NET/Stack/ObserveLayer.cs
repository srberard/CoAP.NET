...
namespace CoAP.Stack
{
    ...
    {
        private static readonly ILogger _logger = LogManager.GetLogger<ObserveLayer>();
            ...
            if (relation != null && relation.Established) {
                if (exchange.Request.IsAcknowledged || exchange.Request.Type == MessageType.NON) {
                    ...
                    if (!Code.IsSuccess(response.Code)) {
                        _logger.LogDebug($"Response has error code {response.Code} and must be sent as CON");
                        response.Type = MessageType.CON;
                        relation.Cancel();
                    } else {
                        // Make sure that every now and than a CON is mixed within
                        if (relation.Check()) {
                            _logger.LogDebug("The observe relation check requires the notification to be sent as CON");
                            response.Type = MessageType.CON;
                        } else {
                ...
                lock (exchange) {
                    ...
                    if (current != null && IsInTransit(current)) {
                        _logger.LogDebug($"A former notification is still in transit. Postpone {response}");
        ...
        private void PrepareSelfReplacement(INextLayer nextLayer, Exchange exchange, Response response) {
            response.Acknowledged += (o, e) => {
                lock (exchange) {
                    ...
                    relation.NextControlNotification = null;
                    if (next != null) {
                        _logger.LogDebug("Notification has been acknowledged, send the next one");
            ...
            response.Retransmitting += (o, e) => {
                lock (exchange) {
                    ...
                    Response next = relation.NextControlNotification;
                    if (next != null) {
                        _logger.LogDebug("The notification has timed out and there is a fresher notification for the retransmission.");
            ...
            response.TimedOut += (o, e) => {
                ObserveRelation relation = exchange.Relation;
                _logger.LogDebug($"Notification {relation.Exchange.Request.TokenString} timed out. Cancel all relations with source {relation.Source}");
                relation.CancelAll();
            };
        }

        private void PrepareTimeout(INextLayer nextLayer, Exchange exchange, Response response) {
            _logger.LogDebug($"PrepareTimeout - for response {response}");
            response.TimedOut += (o, e) => {
                lock (exchange) {
                    ObserveRelation relation = exchange.Relation;
                    _logger.LogDebug($"Notification {relation.Exchange.Request.TokenString} timed out.");
        ...
        private void PrepareReregistration(Exchange exchange, Response response, Action<Request> reregister) {
            ...
            _logger.LogDebug($"Scheduling re-registration in " + timeout + "ms for " + exchange.Request);

            ctx.Restart();
        }

        private class ReregistrationContext : IDisposable
        {
            ...
            private void timer_Elapsed(object sender, ElapsedEventArgs e) {
                ...
                if (request.ObserveRelation != null && !request.ObserveRelation.Canceled && request.ObserveRelation.Reconnect) {
                    ...
                    _logger.LogDebug($"Dropping re-registration for canceled {request}");
                }
            }
        }
    }
