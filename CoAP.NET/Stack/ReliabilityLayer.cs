...
namespace CoAP.Stack
{
    ...
    {
        private static readonly ILogger _logger = LogManager.GetLogger<ReliabilityLayer>();
        private static readonly object _TransmissionContextKey = "TransmissionContext";
        ...
        public override void SendResponse(INextLayer nextLayer, Exchange exchange, Response response) {
            ...
            if (response.Type == MessageType.CON) {
                _logger.LogDebug($"Scheduling retransmission for {response}", response);
                ...
                _logger.LogDebug($"Scheduling timeout for {response}  @ {_nonTimeout}");
        ...
        public override void ReceiveRequest(INextLayer nextLayer, Exchange exchange, Request request) {
            if (request.Duplicate) {
                ...
                if (exchange.CurrentResponse != null) {
                    _logger.LogDebug("Respond with the current response to the duplicate request");
                    base.SendResponse(nextLayer, exchange, exchange.CurrentResponse);
                } else if (exchange.CurrentRequest != null) {
                    if (exchange.CurrentRequest.IsAcknowledged) {
                        _logger.LogDebug("The duplicate request was acknowledged but no response computed yet. Retransmit ACK.");
                        EmptyMessage ack = EmptyMessage.NewACK(request);
                        SendEmptyMessage(nextLayer, exchange, ack);
                    } else if (exchange.CurrentRequest.IsRejected) {
                        _logger.LogDebug("The duplicate request was rejected. Reject again.");
                        EmptyMessage rst = EmptyMessage.NewRST(request);
                        SendEmptyMessage(nextLayer, exchange, rst);
                    } else {
                        _logger.LogDebug("The server has not yet decided what to do with the request. We ignore the duplicate.");
        ...
        public override void ReceiveResponse(INextLayer nextLayer, Exchange exchange, Response response) {
            ...
            if (response.Type == MessageType.CON && !exchange.Request.IsCancelled) {
                _logger.LogDebug("Response is confirmable, send ACK.");
                EmptyMessage ack = EmptyMessage.NewACK(response);
                SendEmptyMessage(nextLayer, exchange, ack);
            }

            if (response.Duplicate) {
                _logger.LogDebug("Response is duplicate, ignore it.");
            } else {
        ...
        public override void ReceiveEmptyMessage(INextLayer nextLayer, Exchange exchange, EmptyMessage message) {
            switch (message.Type) {
                ...
                default:
                    if (_logger.IsEnabled(LogLevel.Warning)) _logger.LogWarning("Empty messgae was not ACK nor RST: " + message);
                    break;
            }
        ...
        private void PrepareTimeout(Exchange exchange, Message msg) {
            ...
            _logger.LogDebug($"Send request, timeout only");
            ...
            public void Cancel() {
                ...
                if (_logger.IsEnabled(LogLevel.Debug)) {
                    if (_exchange.Origin == Origin.Local) {
                        _logger.LogDebug($"Cancel retransmission for --> {_exchange.CurrentRequest}");
                    } else {
                        _logger.LogDebug($"Cancel retransmission for --> {_exchange.CurrentResponse}");
                    }
                }
            }
