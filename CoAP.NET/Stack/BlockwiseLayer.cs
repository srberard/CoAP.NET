...
namespace CoAP.Stack
{
    ...
    {
        private static readonly ILogger _logger = LogManager.GetLogger<BlockwiseLayer>();
        ...
        private void ConfigChanged(object sender, System.ComponentModel.PropertyChangedEventArgs e) {
        ...
        public override void SendRequest(INextLayer nextLayer, Exchange exchange, Request request) {
            if (request.HasOption(OptionType.Block2) && request.Block2.NUM > 0) {
                ...
                _logger.LogDebug("Request carries explicit defined block2 option: create random access blockwise status");
        ...
        public override void ReceiveRequest(INextLayer nextLayer, Exchange exchange, Request request) {
            if (request.HasOption(OptionType.Block1)) {
                ...
                _logger.LogDebug($"Request contains block1 option {block1}", block1);

                BlockwiseStatus status = FindRequestBlockStatus(exchange, request);
                if (block1.NUM == 0 && status.CurrentNUM > 0) {
                    // reset the blockwise transfer
                    _logger.LogDebug("Block1 num is 0, the client has restarted the blockwise transfer. Reset status.");
                ...
                if (block1.NUM == status.CurrentNUM) {
                    ...
                    if (block1.M) {
                        ...
                        _logger.LogDebug("This was the last block. Deliver request");
                ...
                _logger.LogInformation($"[{request.ID}] Sending block {block2.NUM}");
        ...
        public override void ReceiveResponse(INextLayer nextLayer, Exchange exchange, Response response) {
            ...
            if (exchange.Request.IsCancelled) {
                ...
                if (response.Type != MessageType.ACK) {
                    _logger.LogDebug("Rejecting blockwise transfer for canceled Exchange");
            ...
            if (block1 != null) {
                ...
                //SRB
                _logger.LogDebug($"Response acknowledges block " + block1);
                ...
                if (!status.Complete) {
                    ...
                    _logger.LogDebug($"Send next block num = {nextNum}");
                    ...
                    _logger.LogDebug("Response has Block2 option and is therefore sent blockwise");
                }
            }
            ...
            if (block2 != null) {
                ...
                if (block2.NUM == status.CurrentNUM) {
                    ...
                    if (status.IsRandomAccess) {
                        ...
                        _logger.LogDebug("Request the next response block");
                        ...
                        _logger.LogDebug($"Assembled response: {assembled}", assembled);
                    ...
                    _logger.LogWarning($"Wrong block number. Expected {status.CurrentNUM} but received {block2.NUM}" +
        ...
        private void EarlyBlock2Negotiation(Exchange exchange, Request request) {
            ...
            if (request.HasOption(OptionType.Block2)) {
                ...
                _logger.LogDebug($"Request with early block negotiation {block2}. Create and set new Block2 status: {status2}");
        ...
        private BlockwiseStatus FindRequestBlockStatus(Exchange exchange, Request request) {
            ...
            if (status == null) {
                ...
                exchange.RequestBlockStatus = status;
                _logger.LogDebug($"There is no assembler status yet. Create and set new Block1 status: {status}", status);
            } else {
                _logger.LogDebug($"Current Block1 status: {status}", status);
            }
        ...
        private BlockwiseStatus FindResponseBlockStatus(Exchange exchange, Response response) {
            ...
            if (status == null) {
                ...
                _logger.LogDebug($"There is no blockwise status yet. Create and set new Block2 status: {status}", status);
            } else {
                _logger.LogDebug($"Current Block2 status: {status}", status);
            }
