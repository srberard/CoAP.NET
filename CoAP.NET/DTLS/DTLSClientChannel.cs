...
namespace CoAP.DTLS
{
    ...
    internal class DTLSClientChannel : IChannel
    {
        ...
        private readonly ILogger _logger = LogManager.GetLogger<DTLSClientChannel>();
        ...
        public ISession GetSession(System.Net.EndPoint ep) {
            ...
            try {
                ...
                _logger.LogError("Failed to establish a DTLS session");
            }
        ...
        public void Send(byte[] data, ISession sessionReceive, System.Net.EndPoint ep) {
            try {
                ...
                if (session == null) {
                    _logger.LogWarning("Setup a new session - ");
                    ...
                    _logger.LogWarning("Don't send because the sessions are different");
                    return;
                }
                ...
                _logger.LogError($"Error in DTLSClientChannel Sending - {e}");
                throw;
            }
        }
